...
def finish_success(aid, plan, result, verification):
    with transaction() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE nexus.change_requests SET status='rolled_back',recovery_status='recovered',rollback_status='succeeded',recovered_at=NOW(),updated_at=NOW() WHERE change_id=%s",(plan['change_id'],))
