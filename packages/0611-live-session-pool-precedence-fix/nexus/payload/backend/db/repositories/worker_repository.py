    ...
    with transaction() as connection:
        with connection.cursor() as cursor:
            ...
            def score(row: dict[str, Any]) -> tuple:
                ...
                source = str(row["source_system"] or "").lower()
                source_rank = {
                    "seymour-native-stratum": 100,
                    "miningcore": 80,
                    "generic-stratum": 60,
                }.get(source, 20)
                live_session = activity in ACTIVE_STATES and bool(row["connection_confirmed"])

                # Current live session authority wins before historical hashrate.
                # A freshly authorized Seymour session must replace a stale
                # MiningCore/CKPool assignment even before its first share.
                return (
                    live_session,
                    source_rank if live_session else 0,
                    activity_rank,
                    row["last_seen_at"],
            ...
            # Align persisted workloads with the winning current worker session.
            # Historical observations remain in worker/workload JSON and audit
            # history, but stale pool assignments must not remain operational.
            cursor.execute(
                """
                UPDATE nexus.workloads AS workload
                SET
                    pool_id = worker.pool_id,
                    pool_instance_id = worker.pool_instance_id,
                    native_pool_id = worker.native_pool_id,
                    status = CASE
                        WHEN worker.current_session = TRUE
                         AND worker.activity_state IN ('active', 'idle')
                            THEN worker.status
                        ELSE 'offline'
                    END,
                    updated_at = NOW()
                FROM nexus.workers AS worker
                WHERE workload.worker_id = worker.worker_id
                """
            )

            # A workload may accumulate old uses-pool relationships when a miner
            # is reassigned. Keep only the relationship matching the current
            # reconciled worker session active.
            cursor.execute(
                """
                UPDATE nexus.relationships AS relationship
                SET
                    status = CASE
                        WHEN worker.current_session = TRUE
                         AND worker.activity_state IN ('active', 'idle')
                         AND relationship.target_id = worker.pool_instance_id
                            THEN 'active'
                        ELSE 'inactive'
                    END,
                    updated_at = NOW()
                FROM nexus.workloads AS workload
                JOIN nexus.workers AS worker
                  ON worker.worker_id = workload.worker_id
                WHERE relationship.source_type = 'workload'
                  AND relationship.source_id = workload.workload_id
                  AND relationship.relationship_type = 'uses-pool'
                """
            )

            # Asset/worker mines-on relationships from older publishers are not
            # authoritative for live state. Deactivate any relationship whose
            # canonical worker is not the current winning session or whose pool
            # target no longer matches that session. The topology reconciler will
            # publish the canonical active asset -> pool relationship afterward.
            cursor.execute(
                """
                UPDATE nexus.relationships AS relationship
                SET status = 'inactive', updated_at = NOW()
                WHERE relationship.relationship_type = 'mines-on'
                  AND relationship.source <> 'platform-topology-reconciliation'
                  AND NOT EXISTS (
                      SELECT 1
                      FROM nexus.workers AS worker
                      WHERE worker.current_session = TRUE
                        AND worker.activity_state IN ('active', 'idle')
                        AND worker.pool_instance_id = relationship.target_id
                        AND (
                            (relationship.source_type = 'worker'
                             AND relationship.source_id = worker.worker_id)
                            OR
                            (relationship.source_type = 'asset'
                             AND relationship.source_id = worker.asset_id)
                        )
                  )
                """
            )
