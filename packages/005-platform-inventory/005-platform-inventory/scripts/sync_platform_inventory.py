...
def main() -> int:
    ...
    for raw in get_workers():
        source_worker_id = str(raw.get("workerId") or raw.get("id") or raw.get("name") or "")
        if not source_worker_id:
            continue

        asset = assets_by_worker.get(source_worker_id)
        worker_type, hardware_type = classify_worker(raw, asset)

        native_pool_id = str(raw.get("poolId") or raw.get("nativePoolId") or "")
        pool_host = str(raw.get("poolHost") or raw.get("host") or "")
        pool = pools_by_native_host.get((native_pool_id, pool_host))

        if pool is None:
            matches = [p for p in pool_records if p.get("nativePoolId") == native_pool_id]
        ...
        worker = {
            ...
            "displayName": raw.get("displayName") or raw.get("name") or source_worker_id,
            ...
            "identity": {"sourceWorkerId": source_worker_id},
            "observedState": raw,
        }
