#!/usr/bin/env bash
set -euo pipefail
PKG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; ROOT="$(cd "$PKG_DIR/../.." && pwd)"; set -a; source "$ROOT/backend/data/private/cmdb.env"; set +a; export PGPASSWORD="$NEXUS_DB_PASSWORD"; PSQL=(psql -q -h "$NEXUS_DB_HOST" -p "$NEXUS_DB_PORT" -U "$NEXUS_DB_USER" -d "$NEXUS_DB_NAME" -v ON_ERROR_STOP=1 -At)
...
VALUES (
    'VERIFY-047-' || extract(epoch from now())::bigint,
    'Package 047 verification',
...
CREATE_JSON="$(curl -fsS \
    ...
    -d "{
        ...
        \"targetType\":\"host\",
        \"targetId\":\"$NEXUS_HOSTNAME\",
        \"parameters\":{\"transport\":\"local\"},
        \"reason\":\"Package 047 verification\",
        \"requestedBy\":\"package-047\"
    }" \
...
echo 'Package 047 verification PASS'
