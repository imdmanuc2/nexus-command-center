...
BACKUP="$ROOT/packages/backups/047-change-rollback-r3-$STAMP"
SERVICE_USER="${SUDO_USER:-$(id -un)}"

echo "Installing Package 047 Revision 3"
...
python3 -m py_compile "$ROOT/backend/api/server.py" "$ROOT/backend/modules/platform_change_rollback.py"
...
sudo cp "$TMP" /etc/systemd/system/nexus-change-rollback-worker.service
rm -f "$TMP"

sudo systemctl daemon-reload
sudo systemctl enable nexus-change-rollback-worker.service
...
echo "Package 047 Revision 3 installed."
echo "Backup: $BACKUP"
