...
// PACKAGE 062.1: canonical topology is the sole live-edge authority.
function canvasCanonicalLiveEdges(nodes = graph.nodes) {
  /*
   * Live mode is authoritative-session driven. The topology database may
   * retain historical MINES_ON relationships for evidence and Time Machine,
   * but the Canvas must draw exactly one current pool path per physical asset.
   */
  if (timeMachineMode) {
  ...
  const retained = graph.edges
    .filter(edge => canvasRelationshipType(edge) !== "MINES_ON")
  ...
  liveWorkers.forEach(worker => {
    const assetId = String(worker.assetId || "").trim();
    const poolInstanceId = String(
      worker.poolInstanceId ||
      worker.observedState?.currentPool ||
      worker.observedState?.poolId ||
      ""
    ).trim();

    const status = String(worker.status || "").toLowerCase();
    const activityState = String(worker.activityState || "").toLowerCase();
    ...
    if (
      ...
      ["stale", "offline", "retired", "disconnected"].includes(activityState) ||
      !nodeIds.has(assetId) ||
      !nodeIds.has(poolInstanceId)
    ) {
      return;
    }

    const existing = currentByAsset.get(assetId);
    const candidateSeen = Date.parse(
      worker.lastSeenAt ||
      worker.lastConnectedAt ||
      worker.updatedAt ||
      0
    ) || 0;
    const existingSeen = existing
      ? Date.parse(
          existing.lastSeenAt ||
    ...
    if (!existing || candidateSeen >= existingSeen) {
      currentByAsset.set(assetId, worker);
    }
  });

  const currentMiningEdges = Array.from(currentByAsset.values()).map(worker => ({
    source: String(worker.assetId),
    target: String(worker.poolInstanceId),
    type: "MINES_ON",
    label: "Mines On",
    properties: {
      active: true,
      currentSession: true,
      sourceSystem: worker.sourceSystem || "",
      workerId: worker.workerId || "",
      activityStatus: worker.activityState || worker.status || "active",
      currentHashrate: Number(worker.currentHashrate || 0),
      sharesPerSecond: Number(worker.sharesPerSecond || 0),
      lastShareAt: worker.lastShareAt || null
    }
  }));

  return [...retained, ...currentMiningEdges];
}
...
function canvasDisplayMetric(node) {
  ...
  const miningAsset =
    inventoryCategory(node) === "asic" ||
    capabilities.has("crypto-mining") ||
...
function canvasRelationshipActive(edge) {
  if (canvasMiningRelationship(edge)) {
    return canvasRelationshipActivity(edge).active;
  }
...
function canvasWorkerForRelationship(edge) {
  ...
  const sourceAssetId = String(
    sourceProps.assetId ||
    (
      sourceNode?.nodeType === "asset" ||
      String(sourceNode?.id || "").startsWith("asset-")
        ? sourceNode?.id
        : ""
    ) ||
    ""
  ...
  const targetPoolInstanceId = String(
    targetProps.poolId ||
    targetProps.poolInstanceId ||
    targetNode?.id ||
    edge.target ||
    ""
  ).trim();

  const sourceIp = inventoryIp(sourceNode);
  const sourceLabel = String(
    sourceNode?.label ||
    canvasDisplayName(sourceNode) ||
    ""
  ).trim().toLowerCase();

  const poolMatches = liveWorkers.filter(worker => {
    const workerPoolInstanceId = String(
      worker.poolInstanceId ||
      worker.observedState?.poolId ||
      ""
    ).trim();
    ...
    return (
      !targetPoolInstanceId ||
      workerPoolInstanceId === targetPoolInstanceId
    );
  });

  return poolMatches.find(worker => {
    const workerAssetId = String(
      worker.assetId || ""
    ).trim();
    ...
    return (
      (
        sourceAssetId &&
        workerAssetId === sourceAssetId
      ) ||
      (
        sourceLabel &&
        workerLabel === sourceLabel
      ) ||
      (
        sourceIp &&
        workerIp === sourceIp
...
function canvasRelationshipActivity(edge) {
  ...
  if (kind === "shares") {
    ...
    const workerObserved = worker?.observedState || {};
    const edgeProperties = edge?.properties || {};

    const sharesPerSecond = Number(
      worker?.sharesPerSecond ||
      workerObserved.sharesPerSecond ||
      edgeProperties.sharesPerSecond ||
      0
    );

    const relationshipHashrate = Number(
      worker?.currentHashrate ||
      workerObserved.hashrate ||
      edgeProperties.currentHashrate ||
      0
    );

    const activityStatus = String(
      edgeProperties.activityStatus ||
      workerObserved.activityStatus ||
      (
        edgeProperties.active === true
          ? "active"
          : ""
      )
    ).trim().toLowerCase();
    ...
    const hasLiveTelemetry =
      relationshipHashrate > 0 ||
      sharesPerSecond > 0;

    /*
     * MiningCore activity is proven through live worker telemetry.
     * Generic Stratum activity may be explicitly operator-confirmed
     * because many Stratum implementations expose no worker API.
     */
  ...
  if (kind === "rpc") {
    const active =
      String(
        edge.sourceNode?.status || ""
      ).toLowerCase() !== "offline" &&
      String(
        edge.targetNode?.status || ""
      ).toLowerCase() !== "offline";
    ...
    return {
      ...
      active,
      rate: active ? 1 : 0,
      duration: active ? 3.2 : 0,
      particles: active ? 1 : 0
    };
  }
...
async function loadLiveMetrics() {
  try {
    const platform = window.NexusExplorerPlatform;
    if (!platform?.getWorkers) {
      throw new Error("Platform worker client is unavailable");
    }

    const payload = await platform.getWorkers();
    liveWorkers = Array.isArray(payload.workers)
      ? payload.workers
      : [];
  } catch (error) {
    console.warn("Unable to refresh Platform workers.", error);
    /* Preserve last-known-good worker telemetry during a transient failure. */
  }
}
...
async function loadGraph(rebuild = false) {
  try {
    ...
    const health = byId("graphHealth");
    if (health) {
      health.textContent = rebuild ? "Reconciling..." : "Loading...";
    }

    const platform = window.NexusExplorerPlatform;
    if (!platform?.getTopology) {
      throw new Error("Platform topology client is unavailable");
    }

    /*
     * Rebuild remains an explicit reconciliation action. The graph rendered
     * afterward always comes from the canonical PostgreSQL Platform API.
     */
    if (rebuild) {
      try {
        await platform.fetchJson("/api/graph/rebuild");
      } catch (error) {
        console.warn("Graph reconciliation action failed; loading current Platform topology.", error);
      }
    }

    graph = await platform.getTopology();
    ...
    if (health) {
      health.textContent = "Live · Platform";
      health.title = `Source: ${graph.source || "Platform topology"}`;
    }
  } catch (err) {
    const health = byId("graphHealth");
    if (health) health.textContent = "Platform Error";

    const list = byId("graphNodes");
    if (list) {
      list.innerHTML = `<div class="empty-state"><h2>Explorer failed.</h2><p>${err.message}</p></div>`;
    }
  }
}
