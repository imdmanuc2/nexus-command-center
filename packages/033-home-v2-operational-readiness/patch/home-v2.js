...
const HOME_V2_REQUEST_TIMEOUT_MS = 8000;
const HOME_V2_STALE_AFTER_MS = 30000;
const HOME_V2_CRITICAL_STALE_AFTER_MS = 120000;
...
async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    if (error?.name === "AbortError") {
      throw new Error(
        `${url} timed out after ${HOME_V2_REQUEST_TIMEOUT_MS / 1000}s`
      );
    }
...
function renderStatus(fleet) {
  ...
  const isCriticalStale =
    ageMs >= HOME_V2_CRITICAL_STALE_AFTER_MS;
  const isStale =
    ageMs >= HOME_V2_STALE_AFTER_MS;

  badge.textContent = isCriticalStale
    ? "DATA OFFLINE"
    : isStale
      ? "STALE DATA"
  ...
  if (status === "critical" || isCriticalStale) {
  ...
  setText(
    ...
    `${relativeTime(fleet.generatedAt)}${
      isStale ? " · telemetry may be stale" : ""
    }`
  );
...
async function loadFleet() {
  ...
  try {
    ...
    homeV2State.consecutiveFailures += 1;

    if (homeV2State.fleet) {
      console.error(
        "Unable to refresh Fleet API; retaining last known data:",
        error
      );
      ...
      if (badge) {
        badge.textContent =
          homeV2State.consecutiveFailures >= 3
            ? "DATA OFFLINE"
            : "STALE DATA";
        badge.className =
          homeV2State.consecutiveFailures >= 3
      ...
      setText(
        "lastUpdated",
        `Using last known telemetry · retry ${homeV2State.consecutiveFailures}`
      );
    } else {
