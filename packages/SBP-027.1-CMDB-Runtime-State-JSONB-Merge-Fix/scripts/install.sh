...
BACKUP="$ROOT/backups/sbp-0271-$STAMP"
...
cd "$ROOT"
python3 "$PKG/payload/patch_runtime_state_repository.py"
python3 "$PKG/payload/patch_sbp028_acceptance.py"

mkdir -p tests
cp "$PKG/payload/tests/test_sbp0271_jsonb_contract.py" tests/test_sbp0271_jsonb_contract.py
cp "$PKG/payload/tests/test_sbp0271_acceptance_contract.py" tests/test_sbp0271_acceptance_contract.py

echo "Backup: $BACKUP"
echo "SBP-027.1 install: PASS"
