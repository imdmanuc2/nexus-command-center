...
BACKUP="${2:-}"
[[ -f "$BACKUP/backend/db/repositories/seymour_runtime_state_repository.py" ]] || { echo "Invalid SBP-027.1 backup: $BACKUP" >&2; exit 1; }
...
rm -f "$ROOT/tests/test_sbp0271_jsonb_contract.py" "$ROOT/tests/test_sbp0271_acceptance_contract.py"
echo "SBP-027.1 rollback: PASS"
