...
def _storage_inventory(_: dict[str, Any]) -> list[str]:
    return [
        "/usr/bin/lsblk",
        "--json",
        "--bytes",
        "--output",
        "NAME,KNAME,PATH,TYPE,FSTYPE,SIZE,MOUNTPOINTS,MODEL,SERIAL,TRAN,UUID,PKNAME,RO,RM",
    ]


def _mounts(_: dict[str, Any]) -> list[str]:
    return [
        "/usr/bin/findmnt",
        "--json",
        "--output",
...
def _network_interfaces(_: dict[str, Any]) -> list[str]:
    return [
        "/usr/sbin/ip",
        "-j",
        "address",
...
def _listening_ports(_: dict[str, Any]) -> list[str]:
    return [
        "/usr/bin/ss",
        "-H",
        "-lntu",
...
class CapabilityRegistry:
    ...
    def _register_defaults(self) -> None:
        ...
        self.register(CapabilityDefinition("host.storage-inventory", "Read block-device and filesystem inventory.", "low", False, 20, _storage_inventory))
        self.register(CapabilityDefinition("host.mounts", "Read mounted filesystem and network-storage bindings.", "low", False, 20, _mounts))
        self.register(CapabilityDefinition("host.network-interfaces", "Read host network-interface addressing.", "low", False, 20, _network_interfaces))
        self.register(CapabilityDefinition("host.listening-ports", "Read currently listening TCP and UDP ports.", "low", False, 20, _listening_ports))
