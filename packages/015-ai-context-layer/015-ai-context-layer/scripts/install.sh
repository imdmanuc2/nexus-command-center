...
BACKUP_DIR="$PROJECT_ROOT/backend/data/private/package-backups/015-ai-context-layer-$STAMP"
...
psql \
  ...
  -U "$NEXUS_DB_USER" \
  -d "$NEXUS_DB_NAME" \
  -f backend/db/migrations/009_ai_context_layer.sql
...
python3 -m py_compile \
  ...
  backend/modules/platform_context.py \
  backend/jobs/platform_context_job.py \
