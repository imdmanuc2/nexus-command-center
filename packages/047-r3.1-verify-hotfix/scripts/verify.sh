...
CHANGE_ID="$("${PSQL[@]}" -c "INSERT INTO nexus.change_requests (change_number,title,description,capability,rollback_capability,target_type,target_id,asset_id,status,risk_level,requested_by,parameters,created_at,updated_at) VALUES ('VERIFY-047-'||extract(epoch from now())::bigint,'Package 047 verification','Temporary rollback lifecycle verification','host.identity','host.identity','host','localhost','localhost','failed','low','package-047','{}'::jsonb,NOW(),NOW()) RETURNING change_id;")"
